...
public class OrderRefundCauseItemController {
    ...
    public Msg add(@RequestBody RefundCauseItemEntity refundCauseItemEntity) {
        ...
        if (!isEmpty(result)) {
            m = Msg.success().messageData("refundItem", refundCauseItemEntity);
        } else {
            m = Msg.fail();
        }
        return m;
    ...
    public Msg findAll() {
        ...
        if (list.isEmpty()) {
            ...
            msg = Msg.success().messageData("refundItem", list);
        }
        return msg;
    }
    ...
    public Msg findById(@RequestBody Map<String, Integer> param, HttpServletResponse response) {
        ...
        if (byId != null) {
            msg = Msg.success().messageData("refundItem", byId);
        } else {
            msg = Msg.fail();
        }
        return msg;
    ...
    public Msg updateInfo(@RequestBody RefundCauseItemEntity refundCauseItemEntity) {
        ...
        if (up > 0) {
            msg = Msg.success().messageData("refundItem", refundCauseItemEntity);
        } else {
            msg = Msg.fail();
        }
        return msg;
